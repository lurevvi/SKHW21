...
class TestClassPetFriends:
    ...
    def test_add_new_pet_with_valid_data(self, get_key,
                                         name = 'sobak',
                                         animal_type = 'собакабабака',
                                         age = '7',
                                         pet_photo = 'images/sobak.jpg'):
    ...
    def test_successful_delete_self_pet(self, get_key):
        ...
        if len(my_pets['pets']) == 0:
            pf.add_new_pet(get_key, "kot_enot", "некот", "5", "images/kot_enot.jpg")
            _, my_pets = pf.get_list_of_pets(get_key, "my_pets")
...
class TestClassNewPetSimple:
    ...
    def test_add_pet_photo_valid_data(self,
        ...
        if len(my_pets['pets']) == 0:
            pf.add_new_pet_simple (get_key, "КотКокос", "котофей", "10")
            _, my_pets = pf.get_list_of_pets (get_key, "my_pets")
...
class TestFullClassPetsFriends:
    ...
    def test_get_api_key_for_invalid_username(self,
        ...
        """ Проверяем что запрос api ключа с несуществующим именем пользователя
         ...
         'Этот пользователь не найден в базе данных'"""
        ...
        assert status == 403
        assert 'Этот пользователь не найден в базе данных' in result
    ...
    def test_get_api_key_for_invalid_password(self,
        ...
        и в результате содержится строка 'Этот пользователь не найден в базе данных'"""
    ...
    # Test_9
    @pytest.mark.xfail(reason="В базе данных пока не применим filter='1234567'")
    ...
    def test_get_my_pets_with_valid_key(self,
        ...
        if len(my_pets['pets']) == 0:
            pf.add_new_pet_simple(auth_key, 'Игнат', 'Пёс', '18')
